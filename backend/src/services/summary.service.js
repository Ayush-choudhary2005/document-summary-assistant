...
/**
 * Groups selected segments into readable "points": consecutive PROSE
 * sentences (index N followed by N+1, neither one a bullet) are merged into
 * one flowing paragraph, since they're almost certainly one continuous
 * idea. Bullet-marked segments are NEVER merged with a neighbor — each one
 * is a distinct, author-curated point and stays on its own line, which is
 * exactly what the UI needs to stop rendering everything as one blob.
 */
function groupIntoPoints(orderedItems) {
  ...
  for (let i = 1; i < orderedItems.length; i += 1) {
    ...
    const adjacent = current.index === prev.index + 1;
    const bothProse = !prev.isBullet && !current.isBullet;

    if (adjacent && bothProse) {
  ...
  return groups.map((group) => ({
    ...
    isBullet: group.some((item) => item.isBullet),
  }));
}

/**
 * Generates an extractive summary: picks the highest-scoring non-noise
 * segments, groups adjacent prose into paragraphs (keeping bullets
 * separate), then returns those points in original order.
 *
 * Returns `summaryPoints` — an array of `{ text, isBullet }` — for
 * structured rendering, plus a flattened `summary` string for plain-text
 * contexts like exports.
 */
function summarizeExtractive(text, length = 'medium') {
  ...
  if (candidates.length <= MIN_SEGMENTS[length]) {
    const summaryPoints = candidates.map((s) => ({ text: s.text, isBullet: s.isBullet }));
    return {
      summary: summaryPoints.map((p) => p.text).join('\n\n'),
      summaryPoints,
